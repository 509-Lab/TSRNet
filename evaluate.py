...
eval_low_data = []
eval_img_name =[]
eval_low_data_name = glob('./datasets__LIME/*')
...
# fuse with the original input to avoid over-exposure
    ...
    print("[*] every run time: %.4f" % every_time) 
    total_run_time += time.time() - start_time
    save_images(os.path.join(sample_dir, '%s_our.png' % (name)), fusion2)
    
ave_run_time = total_run_time / float(len(eval_low_data))
